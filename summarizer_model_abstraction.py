    ...
    for keyword in keyWordSentenceMapping:
        if len(keyWordSentenceMapping[keyword]) != 0:
            wordsense = get_wordsense(
                keyWordSentenceMapping[keyword][0], keyword)
        if wordsense:
            ...
            try:
                ...
                for idx, choice in enumerate(top4choices):
                    currentQuestion["choices"].append(
                        {"choice": choice, "index": optionchoices[idx], "isCorrect": False})
...
def generateQuestionsFromCorpus(corpus):
    ...
    keyWordSentenceMapping = preProcessCorpusForNet(
        sanitizedText, filteredKeys)
    keyDistractorList = getDistractorFromPreProcessedCorpus(
        keyWordSentenceMapping)
    questions = getQuestionsFromDistractors(
        keyDistractorList, keyWordSentenceMapping)
    return {"questions": questions, "summary": sanitizedText}
